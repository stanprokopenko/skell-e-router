...
The shared package's source `__version__` assignment also reads 3.26.2. Normal Python startup selects LiteLLM 1.83.14, OpenAI 2.24.0, Anthropic 0.116.0, google-genai 2.20.0, tenacity 9.1.4 and requests 2.34.2. Anthropic, OpenAI and requests resolve through user site-packages under `AppData/Roaming/Python/Python311`. Static inspection of the system site-packages instead finds Anthropic 0.84.0 and requests 2.32.5. The rollout owner's [two-location baseline](credential-error-sdk-baseline.json) reconciles these scopes; no SDK drift was found. Preserve both installations and their import priority during router-only replacement.
