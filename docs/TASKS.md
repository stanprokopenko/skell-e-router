...
- [ ] Execute installed rollout after orchestrator release of the [concrete gates and packet](credential-error-rollout.md). Shared Python 3.11 remains on affected v3.26.2 at `d8ae9876fd2f095d5e6e03e11710c6d7a8ddcefe`. Original qualification owner and security fixer are quiet; P2 shared launches are paused. Actual producer holds and parent activation/runtime checks remain required. The network/sampler gate is clear. [Both SDK locations](credential-error-sdk-baseline.json) are recorded and must stay unchanged. Search's stub tests pass 10, digest checks pass 15, and actual installed-router search negative checks pass on 3.26.2 as compatibility evidence only. Other virtual environments require separate work.
- [ ] Orchestrator to assign separate consumer owners for 11 older isolated installations after confirming active use. [Read-only reconciliation](isolated-router-credential-exposure.md) confirms nine stale version-label conflicts already present in pinned source and unsafe provider error handling in all 11 copies. No older runtime was executed or updated, and shared rollout will not repair them.
