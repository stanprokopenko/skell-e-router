...
- [ ] Execute installed rollout after orchestrator release of the [concrete gates and packet](credential-error-rollout.md). Shared Python 3.11 remains on affected v3.26.2 at `d8ae9876fd2f095d5e6e03e11710c6d7a8ddcefe`. All qualification owners and the network gate are clear. Relay owner must enforce its launch pause/drain, and Houston owner must establish its controlled shutdown boundary before release. [Both SDK locations](credential-error-sdk-baseline.json) remain unchanged. Search's stub tests pass 10, digest checks pass 15, and actual installed-router search negative checks pass on 3.26.2 as compatibility evidence only. Other virtual environments require separate work.
