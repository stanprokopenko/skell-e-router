...
- [ ] Follow-up: limited live trial of Jev on chat routing. Implemented in skell-e-web behind the `SKELLE_ROUTING_JEV` switch (off, shadow, on) with gpt-5.6-luna deciding below 0.2 confidence; offline replay 515/562 (91.6%). Live since 2026-09-21 (shadow skipped on Stan's call); the secret is bound and live probes routed correctly. Pending: the seven-day audit, tracked in skell-e-web docs/TASKS.md; record the numbers here when it lands.
...
- [x] Derive `skell_e_router.__version__` from package metadata instead of a separate literal. Done in 3.31.1: `__version__` now comes from `importlib.metadata` (fallback `0.0.0+uninstalled` for uninstalled checkouts), guarded by tests/test_version.py; probe/verify scripts accept the missing literal. See [consumer verification](router-consumers-2026-09-22.md).
...
- [ ] Jev rough cut round two (in progress, branch jev-roughcut-r2, worktree .worktrees/jev-roughcut-r2-2026-09-25). Design: [spec](superpowers/specs/2026-09-26-jev-roughcut-round-two-design.md). Results so far, all with modules on the 18 ladder episodes: route 1 (Jev flags trims, Luna trims) dead at -0.15 SP (docs/jev-real/roughcut-route1-flags.md); route 2 real run, Jev plus gpt-5.6-luna medium on Jev's unsure 25% by margin, 83.63 SP (ceiling 84.06, docs/jev-real/roughcut-hybrid-luna.md), 27 s and $0.06 per episode; prompt breakup f1 (18 yes/no Jev questions plus code features plus v3 score, logistic combiner fitted leave-one-out on the fit six, frozen for held-out) 82.91 SP, 8 s and about $0.09 per episode (docs/jev-real/roughcut-jev-f1.md). Round-one line follows for history.
- [x] Jev rough cut round one, handoff at [docs/handoffs/2026-09-20-jev-roughcut-round-two.md](handoffs/2026-09-20-jev-roughcut-round-two.md); retakes panel wiring and the benchmark page arm handed to a solar-sailer lead via [docs/handoffs/2026-09-20-retakes-panel-jev-pick.md](handoffs/2026-09-20-retakes-panel-jev-pick.md). Round one (2026-09-20) built a Jev-only rough cut in scripts/jev_real/roughcut_jev.py: 80.47 sentence points with modules on the 18 ladder episodes, 17th of 24, 3.2 under the best Luna arm and 6.0 under the shipped Opus agentic, about 6 s and $0.05 per episode. Design and status: [spec](superpowers/specs/2026-09-20-jev-roughcut-design.md); results: docs/jev-real/roughcut-jev-heldout.md. Next levers, pending Stan's call: a paragraph or topic-block pass for the critiques (reuse the production Retakes module's paragraph flags), the over-keeping on greco-2.2-thumbnailing (kept ratio 178%), and the retake pass as a standalone retakes-panel feature in solar-sailer. Round-one spend about $2.40.
