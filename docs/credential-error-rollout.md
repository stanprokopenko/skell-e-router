...
| Houston activation | Network/time gate is clear. Successor `lead-aaf8bc38-0749-4216-8149-2f8f115f8c1d` owns Finish Houston desktop activation from [handoff](../../houston/docs/handoffs/2026-09-05-tldr-desktop-activation.md) `6950339`. Predecessor `lead-7c24d887-559d-4e3c-b257-a86800ebc1f3` is retired. Houston remains open. | The release record must name the successor and require its fresh stop/swap closure receipt. The predecessor's quiet confirmation is not closure evidence. |
...
The selected alternative is a verified new launch context. Old PID 1004's Start Menu/tray ancestry and current registry PATH remain inference, not an inside-process capture. Under root's released window, the Houston successor runs its prepared `controlled-relaunch.ps1 -Phase stop -OldPid 1004`, which uses the approved updater's close/swap operation and leaves Houston closed. Its receipt must confirm the old app and helpers exited before router replacement. No tray-Quit request or relaunch belongs between that stop receipt and the installed router checks. After installed checks, relay release and root's query acceptance, the successor uses `-Phase launch -ExpectedRouterVersion 3.26.3` for the deterministic start. Do not let Electron forward the new launch to the old single instance.
...
Regenerate this operator's identity immediately before activation with `powershell -NoProfile -File scripts/capture_router_operator.ps1 -OperationId <authorized-operation-id>`. It writes `docs/credential-error-operator-current.json` with the current hosted run ID, host/engine PIDs and creation times, host-record hash and outcome-file path. It checks the host is an ancestor of the executing tool, its recorded start matches the live process, and no outcome exists. It prints no pipe name, environment or command line. The [current preparation receipt](credential-error-operator-current.json) is valid only while that same run remains active; returning a final response expires it. The actual activation run must regenerate the receipt and stay active through the offline local sequence. These facts establish live identity, not an observed restart-survival test or installation authorization.
...
[Recovery evidence](credential-error-exact-recovery.json) records a real offline pip upgrade to the accepted 3.26.3 wheel inside an isolated venv, followed by exact restoration of all 41 files. All 887 nonrouter files retained their before hash, `f1a23e1ba9dc7845cbad04bc4e570ac9af4a4f46e50c0cb42e9b0f9398f2b931`. The shared package's files still matched the backup afterward. The original 24 tests did not cover a present METADATA file without Version. The relay operator's exact case reproduced as a ValueError against `f345e6d`; it is corrected now. Twenty-seven recovery tests and 17 subtests pass with no skips.

The added case renames the fixture metadata directory to the declared 3.26.3 directory, writes exactly `Name: skell-e-router` followed by a newline into METADATA, and restores with `--replace-version 3.26.3`. That now restores all 41 files and the original VCS identity in the real isolated backup fixture. Only destination metadata in explicitly declared restore roots can omit Version. Backup metadata remains strict, unrelated Name values still fail, and path/link checks are unchanged. The evidence's `missing_version_case` records the original failure, corrected helper hash and exact passing identity.

Relay operator rerun command from this repository: `../skell-e-router-security/.security-venv/Scripts/python.exe -I scripts/run_security_tests_offline.py tests/test_router_package_recovery.py -q -k metadata_missing_version`. It requires one passing test. Omit `-k metadata_missing_version` for all 27 tests and 17 subtests. No additional blind review round was requested or run.
